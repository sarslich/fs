...
public class HTTPClient {
	 ...
	 * @return
	 */
	public String doGet(boolean isHttps, String url) {
	 ...
	 * @return
	 */
	public JsonResult doPost(boolean isHttps, String url, String data, String contentType) {
		...
		try {
			...
			RequestConfig requestConfig = RequestConfig.custom().setSocketTimeout(20000).setConnectTimeout(10000).build();// 设置请求和传输超时时间
			...
			e.printStackTrace();
			log.error("不支持编码异常："+url,e);
			result.setMessage("不支持编码异常");
		} catch (ClientProtocolException e) {
			e.printStackTrace();
			result.setMessage("协议异常");
			log.error("协议异常：" + url,e);
		} catch (SocketTimeoutException e) {
			e.printStackTrace();
			log.error("响应超时：" + url,e);
			result.setCode("400");//响应超时代码
			result.setMessage("响应超时");
		} catch (ConnectTimeoutException e) {
			e.printStackTrace();
			result.setMessage("连接超时");
			log.error("连接超时：" + url,e);
		} catch (IOException e) {
			e.printStackTrace();
			result.setMessage("Connection refused");
	 ...
	 * @return
	 */
	public String httpRequest(boolean isHttps, String requestUrl, String requestMethod, String outputStr) {
